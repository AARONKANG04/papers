...
parser = argparse.ArgumentParser(description="Train GPT with Muon + AdamW on FineWeb-Edu")
...
def prefetch_batch(split):
    """Prepare a batch asynchronously on a CUDA stream."""
    x, y = get_batch(split)
    return x, y
...
def get_lr(step, base_lr):
    if step < args.warmup_steps:
        return base_lr * (step + 1) / args.warmup_steps
    decay_ratio = (step - args.warmup_steps) / (args.max_steps - args.warmup_steps)
    return base_lr * 0.5 * (1.0 + math.cos(math.pi * decay_ratio))
...
muon_optimizer = Muon(muon_params, lr=args.muon_lr)
adam_optimizer = torch.optim.AdamW(adam_params, lr=args.adam_lr, weight_decay=args.weight_decay)
...
pbar = tqdm(range(args.max_steps), desc="Muon+AdamW")
for step in pbar:
    model.train()

    muon_lr = get_lr(step, args.muon_lr)
    adam_lr = get_lr(step, args.adam_lr)
    ...
    if (step + 1) % args.log_every == 0:
        ...
        pbar.set_postfix(loss=f"{avg:.4f}", muon_lr=f"{muon_lr:.2e}", adam_lr=f"{adam_lr:.2e}")
        running_loss = 0.0
        running_count = 0
...
np.save("logs/muon_loss.npy", np.array(train_losses))
np.save("logs/muon_val_loss.npy", np.array(val_losses))
print(f"\nSaved logs/muon_loss.npy ({len(train_losses)} entries)")
print(f"Saved logs/muon_val_loss.npy ({len(val_losses)} entries)")
